...
void Mol2D::resetNum(loop *l, const vector<int> &a) {
	if (l == NULL) {
		return;
	}
...
void Mol2D::getLoop(vector<res> &v, vector<loop *> &s, const vector<int> &a) {
...
pair<vector<loop *>, int> Mol2D::extend_hinge(loop *src, int hinge_base_pair_num) {
	static int it = 0;
	it++;
