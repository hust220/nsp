...
void ScoreCg::run(const Chain & c) {
	int i, j, l;
	Chain chain;
...
void ScoreCg::train(const Chain &c) {
	int i, j, len;
	Chain chain;
	std::vector<int> t;
...
void ScoreCg::update_counts_stacking(int n1, int n2) {
	int i, j;
	double d;
	int n, a, b;
...
void ScoreCg::update_counts_pairing(int n1, int n2) {
	int i, j;
	double d;
...
double ScoreCg::en_stacking(const Residue &r1, const Residue &r2) {
	return this->en_bp(r1, r2).m_en_stacking;
}

double ScoreCg::en_pairing(const Residue &r1, const Residue &r2) {
	return this->en_bp(r1, r2).m_en_pairing;
}

bool ScoreCg::in_base(int n) {
	return true;
}

Score &ScoreCg::en_bp(const Residue &r1, const Residue &r2) {
	int t1, t2;
	const Residue *p1, *p2;
	Residue *temp1, *temp2;
	double d;
...
//	m_en_stacking += bar(m_freqs_st35, m_weights_st35, t2, t1, parbp.o12_, std::fabs(parbp.theta), 5, 5, 5);
	m_en_pairing += bar(m_freqs_bp, m_weights_bp, t1, t2, parbp.o21_, -parbp.theta, 10, 10, 2);
	m_en_pairing += bar(m_freqs_bp, m_weights_bp, t2, t1, parbp.o12_, -parbp.theta, 10, 10, 2);
	m_en_stacking += bar(m_freqs_st53, m_weights_st53, t1, t2, parbp.o21_, (parbp.theta), 5, 5, 5);
	m_en_stacking += bar(m_freqs_st35, m_weights_st35, t2, t1, parbp.o12_, (parbp.theta), 5, 5, 5);
...
void ScoreCg::print_counts(std::ostream & stream) const {
...
void ScoreCg::print_freqs(std::ostream & stream) const {
	auto foo = [&stream](auto && m) {
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				stream << i << ' ' << j << std::endl;
...
double ScoreCg::en_len(const Chain &c, int beg) {
	double d;
...
double ScoreCg::en_ang(const Chain &c, int beg) {
	double d;
...
double ScoreCg::en_dih(const Chain &c, int beg) {
	double d;
...
double ScoreCg::en_crash(const Residue &r1, const Residue &r2) {
	int i, j;
	double d, e;
