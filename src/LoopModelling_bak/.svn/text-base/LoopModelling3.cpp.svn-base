#include "LoopModelling3.h"

void LoopModelling3::init() {
	delete bound;
	...
	for (int i = 0; i < len; i++) {
		for (int j = i; j < len; j++) {
			if (i == j) {
				...
				bound->data[j][i] = par[(j - i - 1) * 12 + 2 * (i % 6)];
				bound->data[i][j] = par[(j - i - 1) * 12 + 2 * (i % 6) + 1];
			} else {
				bound->data[j][i] = 7;
				bound->data[i][j] = 999;
			}
...
void LoopModelling3::set_base_pairs(loop *src) {
	if (src == NULL) {
		return;
	} else {
		if (src->s.head != NULL) {
				...
				for (int j: {0, 1}) {
					for (int ii = 0; ii < atom_nums_per_chain; ii++) {
						for (int jj = 0; jj < atom_nums_per_chain; jj++) {
							...
							bound->data[orig_pos_chain1 + (orig_pos_chain2 - orig_pos_chain1) * i + ii]
							...
							= helix_par(i * (helix_par.rows() - atom_nums_per_chain) + ii, j * (helix_par.rows() - atom_nums_per_chain) + jj) -
							  err_radius;
							bound->data[orig_pos_chain1 + (orig_pos_chain2 - orig_pos_chain1) * j + jj]
...
RNA *LoopModelling3::run() {
	...
	for (int i = 0; i < resLen; i++) {
		...
		p = new Point[6];
		for (int j = 0; j < 6; j++) {
			p[j].x = backbone->data[i * 6 + j][0];
			p[j].y = backbone->data[i * 6 + j][1];
			p[j].z = backbone->data[i * 6 + j][2];
		}
		...
		if (i != 0 && ss[i] == '(') {
			...
			o = new Point;
			o->x = backbone->data[i * 6 + 5][0];
			o->y = backbone->data[i * 6 + 5][1];
			o->z = backbone->data[i * 6 + 5][2];
		}
	}
rna->print();
	return rna;
...
double LoopModelling3::at(Matr_ *bound, int i, int j, int len) {
    if (j >= len) {
        j -= len;
...
void LoopModelling3::assign(Matr_ *bound, int i, int j, double d, int len) {
    if (j >= len) {
        j -= len;
...
Residue *LoopModelling3::buildNuc(Point *p1, Point *o, double chi, int type) {
