...
int Assemble::find_templates(loop *l) {
    ...
    BuildHelix build_helix;
    build_helix._lib = lib;
    build_helix._type = type;
    if (l->s.head != NULL) {
        if (l == mol.pseudo_head && l->head == NULL) {
            templates[l].second = find_helices(&(l->s), num);
        } else {
            templates[l].second.push_back(build_helix(&(l->s)));
            //templates[l].second = build_helix(&(l->s), 1);
...
vector<Model> Assemble::find_loops(loop *l, int nums) {
    ...
    while (ifile >> loop_item.name >> loop_item.seq >> loop_item.ss >> loop_item.family) {
        if (loop_item.ss == l_ss) {
            ...
            if (loop_item.name.substr(5, 4) == job.substr(0, 4)) {
                if (is_test) {
                    continue;
                } else {
                    loop_item.score += 5;
...
//std::vector<Model> Assemble::create_helices(helix *h, int num) {
//    if (seq.size() < 4) {
