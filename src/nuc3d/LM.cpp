...
std::vector<Model> LM::operator ()(string seq, string ss, string constraint_file, int num) {
    _seq = seq;
    _ss = ss;
...
Model LM::to_all_atom(const MatrixXf &scaffold) {
...
void LM::set_base_pairs(loop *src) {
    if (src == NULL) {
        return;
    } else {
        if (src->s.head != NULL) {
...
R5P LM::get_helix(const helix &h) {
...
R5P LM::create_helix(const string &seq) {
    if (seq.size() < 4) {
...
MatrixXf LM::get_helix_par(const R5P &r5p) {
...
MatrixXf LM::apply_constraints(const MatrixXf &scaffold) {
...
std::vector<std::vector<std::vector<int>>> LM::read_constraints_file() {
...
MatrixXf LM::read_constraints_pos(std::string type, int layer_nums) {
...
MatrixXf LM::read_constraints_par(std::string type, int layer_nums) {
