...
#define JN_TODO \
    do {\
        std::cerr << "There is a function to be completed!" << std::endl;\
        throw "exit";\
    } while(0)
...
using BasicStr = std::basic_string<_CharType, _CharTraits>;
...
template<typename _Type>
using UPtr = std::unique_ptr<_Type>;
template<typename _Type>
using UP = UPtr<_Type>;

template<typename _Fty>
using Fn = std::function<_Fty>;
template<typename _Fty>
using Function = std::function<_Fty>;

template<typename _Type, I _N>
using A = std::array<_Type, _N>;
template<typename _Type, I _N>
using Array = std::array<_Type, _N>;
template<int _N>
using An = A<N, _N>;
template<int _N>
using Ai = A<I, _N>;
...
template<typename _Type>
using L = std::list<_Type>;
template<typename _Type>
using List = std::list<_Type>;
using Ln = L<N>;
using Li = L<I>;
using Ld = L<D>;
using Lf = L<F>;
...
template<typename _Type>
using T = std::set<_Type>;
template<typename _Type>
using Set = std::set<_Type>;
using Tn = T<N>;
using Ti = T<I>;
using Td = T<D>;
using Tf = T<F>;
...
using M = std::map<_KeyType, _Type>;
template<typename _KeyType, typename _Type>
using Map = std::map<_KeyType, _Type>;
template<typename _Type>
using Mn = M<N, _Type>;
template<typename _Type>
using Mi = M<I, _Type>;
