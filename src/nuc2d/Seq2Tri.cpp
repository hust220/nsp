...
Seq2Tri::InfoList Seq2Tri::operator ()(std::string seq) {
    _seq = seq;
    int len = seq.size();
    ...
    for (auto &&info: sub_info_list) {
    	for (int i = 0; i < info.first.size(); i++) {
    		if (info.first[i].size() == 0) {
    			std::cout << '.';
    		} else if (info.first[i][0] > i && info.first[i][1] > i) {
    			std::cout << '<';
    		} else if (info.first[i][0] < i && info.first[i][1] > i) {
    			std::cout << 'X';
    		} else if (info.first[i][0] < i && info.first[i][1] < i) {
...
Seq2Tri::InfoList Seq2Tri::best_info(const std::vector<int> &seq) {
	InfoList info_list;
...
Seq2Tri::TupleInfo Seq2Tri::strand_info(const std::vector<int> &seq) {
...
Seq2Tri::TupleInfo Seq2Tri::score(const Seq2Tri::TupleInfo &info, int m) {
...
Seq2Tri::TupleInfo Seq2Tri::score(const Seq2Tri::TupleInfo &info1, const Seq2Tri::TupleInfo &info2, int a, int b, int c) {
...
double Seq2Tri::stack_energy(char m, char n) {
	int a = _convert[m];
	int b = _convert[n];
	return _stack_energy(a, b);
}

std::pair<double, bool> Seq2Tri::pair_energy(char m, char n) {
	int a = _convert[m];
	int b = _convert[n];
	if (a == 0 && b == 1) {
		return {-484.06, true};
...
std::pair<double, bool> Seq2Tri::tuple_energy(char a, char b, char c) {
	auto first = pair_energy(a, b);
	auto second = pair_energy(b, c);
	return {first.first + second.first, first.second && second.second};
}

Seq2Tri::InfoList Seq2Tri::sub_info(const std::vector<int> &seq, double cutoff) {
    InfoList info_list;
