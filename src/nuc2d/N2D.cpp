...
void N2D::setTree(vector<res> &v0, int view) {
	int i, j, temp;
...
void N2D::resetNum(loop *l, const vector<int> &a) {
	if (l == NULL) {
		return;
	}
...
void N2D::setSeq(loop *l, string str) {
	res *tempRes;
	bp *b;
...
void N2D::printTree(loop *head, int spaces) {
	bp *b;
	res *r;
...
void N2D::getLoop(vector<res> &v, vector<loop *> &s, const vector<int> &a) {
//void N2D::getLoop(vector<res> &v, vector<loop> &s, int *a) {
	...
	// set tree relation
	//num = l->getType();
//cout << "l->getSS(): " << l->getSS() << endl;
//cout << "l->hinges.size(): " << l->hinges.size() << endl;
	num = l->hinges.size();
	i = s.size() - num;
	//i = s.size() - num + 1;
	if (s.size() != 0 && num != 0) {
...
//			if (src->son != NULL) {
//				loop *temp_loop = src->son;
//				for (; temp_loop->brother != NULL; temp_loop = temp_loop->brother);
//				temp_loop->brother = src->brother;
//				loop *temp_son = src->son;
//				*src = *src->son;
//				delete temp_son;
//				return pair<vector<loop *>, int>(loop_list, 0);
//			} else if (src->brother != NULL) {
//				loop *temp_brother = src->brother;
//				*src = *src->brother;
//				delete temp_brother;
//				return pair<vector<loop *>, int>(loop_list, 0);
//			} else {
//				return pair<vector<loop *>, int>(loop_list, 1);
//			}
...
//if (view) {
//	cout << it << ' ' << "\tres_index: " << res_index << endl;
//	cout << it << ' ' << "\tsrc->hinges[hinge_index].first: " << src->hinges[hinge_index].first << endl;
//}
...
//				new_loop->hinges.push_back(pair<int, int>(
//					hinge_base_pair_num - 1, hinge_base_pair_num));
if (view) cout << it << ' ' << src->getSS() << " new_loop->hinges.size(): " << new_loop->hinges.size() << endl;
		}

//		} else {
//if (view) cerr << it << ' ' << "return" << endl;
//			return pair<vector<loop *>, int>(loop_list, 0);
//		}
...
int N2D::pseudo_tree(loop *root, loop *src, set<loop *> loop_set, loop *pseudo) {
	if (src == NULL) {
		return 0;
	} else {
		loop *son= src->son;
