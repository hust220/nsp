...
int main(int argc, char **argv) {
    ...
    if (par["global"][0] == "dist") {
        std::cout << jian::geometry::distance(vec[0], vec[1]) << std::endl;
    } else if (par["global"][0] == "ang") {
        std::cout << jian::geometry::angle(vec[0], vec[1], vec[2]) << std::endl;
    } else if (par["global"][0] == "dih") {
        std::cout << jian::geometry::dihedral(vec[0], vec[1], vec[2], vec[3]) << std::endl;
    } else if (par["global"][0] == "chir") {
        std::cout << jian::geometry::chirality(vec[0], vec[1], vec[2], vec[3]) << std::endl;
    } else {
    }
    return 0;
}
