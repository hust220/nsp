...
public:
    ...
    Hinges ss_to_hinges(const std::string &ss) {
        ...
        if (!stack.empty()) throw ("jian::ss_to_hinges error! ss: "s + ss).c_str();
        return hinges;
    }
    ...
    Model make_junction(const Hinges &hinges) {
        ...
        for (auto &helix : helices) Trace::log(helix.theta, ':', helix.phi, ' '); Trace::log('\n');
        TRACE_OUT;
    ...
    void adjust_helix(Model &model, const O &o, const N &n, val_t theta_o, val_t phi_o, val_t theta_n, val_t phi_n) {
        ...
        Trace::log(o[0], ':', o[1], ':', o[2], ' '); Trace::log(n[0], ':', n[1], ':', n[2], '\n');
