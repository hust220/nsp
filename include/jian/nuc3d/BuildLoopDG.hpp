...
public:
    ...
    void init(const std::string &seq, const std::string &ss, const std::string &c = "") {
        ...
        SSTree ss_tree;  ss_tree.make(seq, ss); 
    ...
    Model to_all_atom(const MatrixXd &c) {
        ...
        Model m; m.push_back(chain); return m;
    }
    ...
    void set_bound_helix(MatrixXd &b, DihBound &d, const helix &h) {
        int len = 0; std::deque<int> s1, s2; HELIX_EACH(h, len++; s1.push_back(BP->res1.num-1); s2.push_back(BP->res2.num-1));
