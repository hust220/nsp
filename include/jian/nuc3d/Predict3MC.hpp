...
class Predict3MC : public virtual MC, public BasicPredict3D, public JobPredict3D, public virtual Rand {
public:    
    using Mat = MatrixXd;
    ...
    int _mc_index;
    std::array<int, 4> _moved_residues;
    ...
    Predict3MC(const Par &par) : JobPredict3D(par) {
...
//                    _is_virtual[L] = true;
    ...
    double mc_partial_energy() {
        ...
        double e = 0;
        double d;
        mc_partial_energy_bond(e, d);
        mc_partial_energy_distance(e, d);
        mc_partial_energy_dihedral(e, d);
        return e;
    }

    void mc_partial_energy_bond(double &e, double &d) {
            ...
            if (i <= p[1] || i >= p[2]) {
                for (int j = 0; j < _seq.size(); j++) {
                    if ((j < p[0]) || (j > p[1] && j < p[2]) || (j > p[3])) {
                        d = geom::distance(residue_center(_pred_chain[i]), residue_center(_pred_chain[j]));
                        if (i - j == 1 || j - i == 1) e += 100 * square(d - 6.1);
                        else if (i - j == 2 || j - i == 2) e += 10 * square(d - 11.7);
    ...
    void mc_partial_energy_distance(double &e, double &d) {
        for (auto && row : _constraints.distances) {
            if (is_selected(row.key[0]) ^ is_selected(row.key[1])) {
    ...
    void mc_partial_energy_dihedral(double &e, double &d) {
    ...
    void mc_sample() {
        ...
        if (rand() < 0.5) {
            ...
            for (int i = _moved_residues[0]; i <= _moved_residues[3]; i++) {
                if (i <= _moved_residues[1] || i >= _moved_residues[2]) {
                    for (auto && atom : _pred_chain[i]) {
                        atom[index] += dist;
                    }
                }
