...
//    MatrixXf read_constraints_par(std::string, int);
    ...
    void set_helix_anchors() {
        ...
        std::sort(res_list.begin(), res_list.end(), [](const nuc2d::res &res1, const nuc2d::res &res2){return res1.num < res2.num;});
    ...
    void set_helix_coords(nuc2d::loop *src) {
        for (bp *b = src->s.head; b != NULL; b = b->next) {
            if (std::count_if(_helix_anchors.begin(), _helix_anchors.end(), [&](const std::vector<nuc2d::res> &vec){
                return std::count_if(vec.begin(), vec.end(), [&](const nuc2d::res &r2){
                    return b->res1.type == r2.type && b->res1.name == r2.name && b->res1.num == r2.num;});})) {
                std::cout << b->res1.num << ' ' << b->res1.type << ' ' << b->res1.name << std::endl;
            }
            if (std::count_if(_helix_anchors.begin(), _helix_anchors.end(), [&](const std::vector<nuc2d::res> &vec){return std::count_if(vec.begin(), vec.end(), [&](const nuc2d::res &r2){return b->res2.type == r2.type && b->res2.name == r2.name && b->res2.num == r2.num;});})) {
                std::cout << b->res2.num << ' ' << b->res2.type << ' ' << b->res2.name << std::endl;
            }
        }
        for (nuc2d::res *r = src->head; r != NULL; r = r->next) {
            if (std::count_if(_helix_anchors.begin(), _helix_anchors.end(), [&](const std::vector<nuc2d::res> &vec){return std::count_if(vec.begin(), vec.end(), [&](const nuc2d::res &r2){return r->type == r2.type && r->name == r2.name && r->num == r2.num;});})) {
