...
public:
    ...
    void make(const std::string &seq, const std::string &ss, int hinge = 2) {
        Trace::log("## Make Secondary Structure Tree\n");
        if (NucSS::seq_match_ss(seq, ss)) {
            head = set_tree(ss, hinge);
            read_seq(head, seq, ss);
        } else {
            throw Error("The sequence and the secondary structure don't match!");
        }
    }

    loop *set_tree(const std::string &ss, int hinge) {
        std::deque<res> v; EACH((c, i), ss, v.push_back(res(c, i+1)));
        std::vector<loop *> ls; while (find_hairpin(v, ls, hinge));
        return ls.back();
    }
