...
namespace jian {
	...
	public:
		...
		template<typename NumType, typename RandomIt, typename F>
		static MatX<NumType> *to_mat(RandomIt && first, RandomIt && last, F && dist) {
			int i, j, size;
			NumType d;
			MatX<NumType> *mat;

			size = std::distance(first, last);
			mat = new MatX<NumType>(size, size);
