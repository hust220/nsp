...
public:
    ...
    Point(double xx, double yy, double zz, int i = 0) {
        x = xx;
        y = yy;
        z = zz;
        type = i;
    ...
    static Point *normalVector(Point *p1, Point *p2, Point *p3) {
    ...
    static double angle(Obj<Point> p1, Obj<Point> p2, Obj<Point> p3) {
    ...
    static double dihedral(Point &p1, Point &p2, Point &p3, Point &p4) {
        return dihedral(&p1, &p2, &p3, &p4);
    }

    static double dihedral(Obj<Point> p1, Obj<Point> p2, Obj<Point> p3, Obj<Point> p4) {
    ...
    static Point *grow(Point *pos1, Point *pos2, double angle, double dihedral, double bondLen) {
    ...
    static Point *rotate(Point *pos1, Point *pos2, Point *pos3, double dihedral, double bondLen1, double bondLen2) {
    ...
    static double gmMin(double a, double b, double c, double d1, double d2, double e1, double e2) {
        ...
        if (e1 <= temp1 && d1 <= temp2) { return 0;
        } else if (e1 <= temp1 && d1 > temp2) { return fabs(d1 - c);
        } else if (e1 > temp1 && d1 <= temp2) { return fabs(e1 - a);
        } else { return Point::gmMin(a, b, c, d1, e1);
        }
    }
    ...
    Point *rotate(Point *l1, Point *l2, double t) {
        Point *p = new Point;
    ...
    Point *rotate(Point *l, double t) {
        Point *p = new Point;
        return rotate(p, l, t);
    }

    static Point *t2c(double *l, double *a, double *t, int n) {
        if (n < 4) {
    ...
    static Point *t2c(double *l, double *a, double *t, Point *p_o, Point *p_n, int n) {
        if (n < 4) {
            cerr << "Too few points!" << endl;
            exit(1);
        }
    ...
    static void rotate(Point *p, int len, Point beg, Point end, double angle) {
        for (int i = 0; i < len; i++) {
            p[i].x -= beg.x;
            p[i].y -= beg.y;
            p[i].z -= beg.z;
    ...
    static void coincide(Point *p, int len, Point o, Point n) {
    ...
    static double chirality(Point *p1, Point *p2, Point *p3, Point *p4) {
    ...
    static double chirality(const Point &p1, const Point &p2, const Point &p3, const Point &p4) {
        Point a(p1.x - p4.x, p1.y - p4.y, p1.z - p4.z);
        Point b(p2.x - p4.x, p2.y - p4.y, p2.z - p4.z);
        Point c(p3.x - p4.x, p3.y - p4.y, p3.z - p4.z);
        Point d(b.y * c.z - b.z * c.y, b.z * c.x - b.x * c.z, b.x * c.y - b.y * c.x);
