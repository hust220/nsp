...
public:
    ...
    PSB2AA() {
        ...
        EACH_SPLIT_LINE((_path + "inf.txt").c_str(), " ",
			...
			for (int i = 0; i < num_atoms; i++) for (int j = 0; j < 3; j++) {
				(*m)(i, j) = std::stod(F[i * 3 + j + 1]);
			}
    ...
    auto run(T &&coord, U &&frag) {
        ...
        for (int i = 0; i < len - num_atoms + 1; i += 3) {
			...
			for (int j = 0; j < num_atoms; j++) for (int k = 0; k < 3; k++) {
			...
			*/
			...
			if (i == 0) for (int j = 0; j < _num_residues_frag - 1; j++) {
