...
class AssembleImpl : public BasicPredict3D, public JobPredict3D {
public:    
    ...
    AssembleImpl(const Par &par) : JobPredict3D(par) {
        Debug::print("# Check input\n");
        if (!NucSS::check_ss(_ss)) throw "The secondary structure includes illegal characters!";

        Debug::print("# Construct 2D Structure Tree\n");
        _ss_tree.make(_seq, _ss, _hinge);

        Debug::print("# Set Virtual Loops\n");
        set_virtual_loops();

        Debug::print("# Searching Templates\n");
        find_templates();

        Debug::print("# Set the Loops to be Sampled\n");
        set_loops_sampled();
...
//                    _is_virtual[L] = true;
    ...
    Model predict() {
        ...
        return transform(residues_to_model(std::move(residues)), _seq, _type);
    }
...
//                        build_loop_dg.init(L->seq(), NucSS::lower_ss(L->ss())); _templates[L].first = build_loop_dg();
//                        _templates[L].first = sample_loop(L, _templates[L].first);
                    }
                } else {
    ...
    Chain make_strand(T &&residues, U &&vec) {
        ...
        Debug::print("make strand ("); for (auto i : vec) Debug::print(i, ' '); Debug::print(")...\n");
        Mat a, b;
        if (vec[0] >= 2) {
            a.resize(2, 3);
            for (int i = 0; i < 3; i++) {
    ...
    void print_records() {
        Debug::print("Records searching results:\n");
        LOOP_TRAVERSE(_ss_tree.head(),
            Debug::print("Hairpin(", L, "):\n", 
